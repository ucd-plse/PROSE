...
FMS_BLD_DIR="${TUNING_REPO_ROOT}"/build/intel/shared/gptl
MOM6_BLD_DIR="${TUNING_REPO_ROOT}"/build/intel/ocean_only/gptl
...
if test -f "$FMS_BLD_DIR/libfms.a"; then
  ...
  make clean
  make NETCDF=3 REPRO=1 GPTL=1 libfms.a
fi
...
../../../../src/mkmf/bin/mkmf -t $INTEL_MKMF_TEMPLATE -o '-I../../shared/gptl' -p MOM6 -l '-L../../shared/gptl -lfms' -c '-Duse_libMPI -Duse_netCDF -DSPMD' path_names
make NETCDF=3 REPRO=1 GPTL=1 $TARGET_MOM6_OBJ
...
echo "Successfully built MOM6 with INTEL and flags for GPTL."