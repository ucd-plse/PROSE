...
ifeq ($(compiler),gnu)
  ...
  $(warning (INFO) Corresponding compilers and flags found in cmplrflags_gcov.mk.)
  ifneq ($(FOUND),TRUE)
     FOUND := TRUE
  else
     MULTIPLE := TRUE
...
ifeq ($(compiler),gnu)
  ...
  FC		:=  gfortran
  PFC		:=  mpif90
  FFLAGS1	:=  $(INCDIRS) -O2 -mcmodel=medium -ffixed-line-length-none -march=k8 -m64 -fdefault-real-8 -fdefault-double-8 -fprofile-arcs -ftest-coverage
  FFLAGS2	:=  $(FFLAGS1)
  FFLAGS3	:=  $(FFLAGS1)
  DA		:=  -DREAL8 -DLINUX -DCSCA
  DP		:=  -DREAL8 -DLINUX -DCSCA -DCMPI -DHAVE_MPI_MOD
...
#  endif
  ...
  CLIBS	:=
  MSGLIBS	:=
  $(warning (INFO) Corresponding machine found in cmplrflags_gcov.mk.)
  ifneq ($(FOUND),TRUE)
     FOUND := TRUE
  else
     MULTIPLE := TRUE
...
ifeq ($(compiler),intel)
  ...
  FC            :=  ifort
  PFC           :=  mpif90
  FFLAGS1       :=  $(INCDIRS) -O2 -FI -assume byterecl -132 -xSSE4.2 -assume buffered_io -r8
...
########################################################################
ifneq ($(FOUND), TRUE)
     $(warning (WARNING) None of the platforms in cmplrflags_gcov.mk match your platform. As a result, the specific compilers and flags that are appropriate for you could not be specified. Please edit the cmplrflags_gcov.mk file to include your machine and operating system. Continuing with generic selections for compilers.)
  PPFC	        := f90
  FC	        := f90
  PFC	        := mpif90
  FFLAGS1	:=  $(INCDIRS)
...
ifeq ($(MULTIPLE),TRUE)
     $(warning (WARNING) More than one match in cmplrflags_gcov.mk. This may result in the wrong compilers being selected. Please check the cmplrflags_gcov.mk file to ensure that only one set of compiler flags is specified for your platform.)
endif
